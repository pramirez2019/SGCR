...
namespace WindowsFormsAddUser
{
    public partial class ABMCategorias : Form
    {
        ...
        private void UpdatecategBtn_Click(object sender, EventArgs e)
        {
            DialogResult dr = MessageBox.Show("Do you want to modify this Categoria?", "Question....", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (dr == DialogResult.Yes)
            {
                obj.UpdateCategoria(Convert.ToInt32(idcategTxt.Text), namecategTxt.Text, descripcategTxt.Text);
            }
        }

        private void DeletecategBtn_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                obj.DeleteCategoria(namecategTxt.Text.Trim());
            }
        }
        ...
        private void DescripcategTxt_TextChanged(object sender, EventArgs e)
        {
            if (descripcategTxt.Text == "")
            {
                descripcategTxt.BackColor = Color.Firebrick;
            }
            else
            {
                descripcategTxt.BackColor = Color.White;
            }
        }
